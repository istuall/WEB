...
//        userDao.insertUser();
    ...
    public List<UserDto> selectUserList(String username) throws SQLException, ClassNotFoundException {
        //获得连接对象
    ...
    public UserDto selectUserByUsername(String username) throws SQLException, ClassNotFoundException {
        //获得连接对象
        Connection conn = DBUtil.getConnection();
        //查询的sql语句
    ...
    public void insertUser(UserDto userDto) throws Exception{
        ...
        //插入语句
        String insertSql = "insert into user(username,password,nickname,sex,age,address,insert_date) " + "values (?,?,?,?,?,?,CURDATE())";
        //获得sql语句预处理平台
        ...
        //关闭连接
        DBUtil.close(connection,pds,null);
    }
    public void updateUser(UserDto userDto) throws Exception {
        Connection connection = DBUtil.getConnection();
        String updateSql = "UPDATE user SET username = ?, password = ?, nickname = ?, sex = ?, age = ?, address = ? WHERE id = ?";
        PreparedStatement pds = connection.prepareStatement(updateSql);
        pds.setString(1, userDto.getUsername());
        pds.setString(2, userDto.getPassword());
        pds.setString(3, userDto.getNickname());
        pds.setString(4, userDto.getSex());
